...
app.put('/api/keys/:id/status', auth, requireAdmin, async (req, res) => {
  ...
  if (error) return res.status(500).json({ error: error.message });
  await supabase.from('audit_logs').insert({ admin_id: req.user.id, action: 'update_key_status', target: id });
...
app.delete('/api/keys/:id', auth, requireAdmin, async (req, res) => {
  ...
  if (error) return res.status(500).json({ error: error.message });
  await supabase.from('audit_logs').insert({ admin_id: req.user.id, action: 'delete_key', target: id });
  res.json({ success: true });
});
...
app.get('/api/admins', auth, requireOwner, async (req, res) => {
  const { data, error } = await supabase.from('admins').select('id, username, role, is_active, created_at');
  if (error) return res.status(500).json({ error: error.message });
  res.json(data || []);
});
...
app.post('/api/admins', auth, requireOwner, async (req, res) => {
  ...
  const { data, error } = await supabase.from('admins').insert({
    username, password_hash: hash, role: role || 'admin'
  }).select().single();
...
app.get('/api/stats', auth, async (req, res) => {
  const [totalKeys, activeKeys, totalUsers] = await Promise.all([
    ...
    supabase.from('keys').select('*', { count: 'exact', head: true }).eq('status', 'active').gt('expires_at', new Date().toISOString()),
